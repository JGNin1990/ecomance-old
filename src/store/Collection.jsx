...
const Collection = ({ children }) => {
  ...
  const searchingFunction = (search) => {
    setSearchedItems(() => ({
      value: items.value.filter((element) => {
  ...
  useEffect(() => {
    ...
    fetch("https://fakestoreapi.com/products")
      ...
      .then((json) =>
        setItem((prev) => {
          return { ...prev, isLoading: false, value: json };
        })
      )
      .catch((e) => {
    ...
    setTotal(
      total.map((each, index) =>
        index === i
          ? {
              ...each,
              price: each.price + ogp,
              selectedNum: each.selectedNum + 1,
            }
  ...
  const minusHandle = (i, ogp) => {
    setTotal(
      total.map((each, index, prev) =>
        index === i && each.selectedNum !== 1
          ? { price: each.price - ogp, selectedNum: each.selectedNum - 1 }
          : { ...prev, price: each.price, selectedNum: 1 }
      )
    );
  };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        delObj.current.childNodes.forEach((eachDiv) => {
          eachDiv.id === index &&
            eachDiv.classList.add("animate__rotateOutDownLeft");
        });
        setTimeout(() => {
